...
uploaded_xml = st.sidebar.file_uploader("Baggerfeldgrenzen (XML mit Namespace)", type=["xml"])
...
baggerfelder = []  # 👉 Initialisierung: leeres Array
if uploaded_xml:
    try:
        baggerfelder = parse_baggerfelder(uploaded_xml, epsg_code if 'epsg_code' in locals() else "EPSG:25832")
        st.sidebar.success(f"{len(baggerfelder)} Baggerfelder geladen")
    except Exception as e:
        st.sidebar.warning(f"Baggerfelder konnten nicht geladen werden: {e}")
...
#=== Automatische Erkennung des Koordinatensystems (UTM, GK, RD) =================================================
    ...
    with tab2:
        ...
        if 'df' in globals() and not df.empty:
            ...
            def format_tooltip(row, soll_key):
                zeit = row["timestamp"].strftime("%H:%M:%S")
                tooltip = f"🕒 {zeit}"
...
#==== Zeit-Auswertung ================================================================
    ...
    with tab3:
        ...
        with st.sidebar.expander("Einstellungen - Zeit-Auswertung"):
        ...
        def to_dezimalstunden(td):
            try:
                ...
                return "-"    
        # Großer Titel für "Auswahl der Kriterien"
        #st.markdown("<h3 style='font-size: 24px'>🕒 Auswahl der Kriterien</h3>", unsafe_allow_html=True)
